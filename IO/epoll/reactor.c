...
{
    int fd; // 要监听的文件描述符 
    int events; //对应的监听事件 EPOLLIN和EPOLLOUT(读或者写)
    void *arg; // 指向自己结构体指针
    void (*call_back)(int fd,int events,void* arg);// callback函数
    int status; // 是否在监听1--> 在红黑树上（监听） 0-->不在（不监听）
    char buf[BUFLEN];
    int len ;
...
/*
 *封装一个自定义事件，包括fd，这个fd的回调函数，还有一个额外参数项目
...
void eventadd(int efd,int events,struct myevent_s *ev)
{
    ...
    int op = 0;
    epv.data.ptr = ev;  // ptr指向一个结构体，之前epoll模型红黑树上挂载的是文件描述符cfd和lfd，现在是ptr指针
    epv.events = ev->events = events; // EPOLLIN 或者  EPOLLOUT
    if(ev->status == 0){ // status 说明文件描述符是否在红黑树上0不在 ，1在
...
void eventdel(int efd,struct myevent_s *ev){
    struct epoll_event epv = {0,{0}};
    if(ev->status != 1) // 如果fd没有添加到监听树上们就不用删除，直接返回
        return;
    epv.data.ptr = NULL;
    ev->status = 0;
...
//当有文件描述符就绪，epoll 返回，调用该函数与客户端建立联系
void acceptconn(int lfd,int events,void *arg){
    ...
    do{
        for(i = 0;i < MAX_EVENTS;i++){ //从全局数组g_events中找一个空闲的元素，类似select中招值为-1的元素
            if(g_events[i].status == 0)
            break;
        }
        if(i == MAX_EVENTS) //超出连接限制
        {
...
void recvdata(int fd,int events,void* args){
    struct myevent_s *ev = (struct myevent_s*) args; // 指针强制类型转换
    int len;
    len = recv(fd,ev->buf,sizeof(ev->buf),0); // 读取客户端发来的数据
    eventdel(g_efd,ev);
    if(len > 0)
    {
        ev->len = len;
...
void initlistensocket(int efd,short port){
    ...
    eventset(&g_events[MAX_EVENTS],lfd,acceptconn,&g_events[MAX_EVENTS]); // 接客
...
int main()
{
    ...
    int i;
    while(1){
        //调用epoll_wait等待接入的客户端事件,epoll_wait传出的是满足监听条件的那些fd的struct epoll_event类型
